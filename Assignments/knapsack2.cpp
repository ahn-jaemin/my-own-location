...
int stealAsMuchAsPossible(std::vector<int>& weights, std::vector<int>& values, std::vector<bool>& inKnapsack, int capacity, int i, std::string indent){
    ...
    if (capacity < weights.at(i)){
        return stealAsMuchAsPossible(weights, values, inKnapsack, capacity, i + 1, indent + "   ");
    }
    ...
    option1 = stealAsMuchAsPossible(weights, values, inKnapsack, capacity, i + 1, indent + "   ");
    option2 = values.at(i) + stealAsMuchAsPossible(weights, values, inKnapsack, capacity - weights.at(i), i + 1, indent + "   ");
...
int main(int argc, char* argv[]){
    ...
    totValue = stealAsMuchAsPossible(weights, values, inKnapsack, capacity, i, "   ");
